...
function getProductUnits(p) {
    ...
    if (!p.baseUnitCode) {
        const myCode = p.code;
        const myBarcode = p.barcode;
        products.forEach(dp => {
            if (dp.baseUnitCode && (dp.baseUnitCode === myCode || dp.baseUnitCode === myBarcode) && dp.code !== p.code) {
                units.push({
    ...
    if (p.baseUnitCode) {
        const baseProduct = products.find(bp => bp.code === p.baseUnitCode || bp.barcode === p.baseUnitCode);
        if (baseProduct) {
            ...
            products.forEach(dp => {
                if (dp.code !== p.code && dp.code !== baseProduct.code && dp.baseUnitCode &&
                    (dp.baseUnitCode === baseProduct.code || dp.baseUnitCode === baseProduct.barcode)) {
...
function calculateTotals() {
    ...
    // Smart-sync discountInput with item-level discounts
    const discInput = document.getElementById('discountInput');
    if (discInput) {
        const discountedItems = cart.filter(c => c.discAmt && c.discAmt > 0);
        if (discountedItems.length === 0) {
            // No item discounts: reset to 0
            discInput.value = 0;
    ...
    const rawDisc = parseFloat(discInput?.value || 0);
    let discount = 0;
    if (discountMode === '%') {
...
function selectPayMethod(method) {
    ...
    if (tab.payMethods.length === 1) {
        // Single method → show its sub-panel, hide split panel
        ...
        // Multi method → show split panel, hide normal pay input
        renderSplitPanel();
        const payRow = document.getElementById('payAmountRow');
...
function renderSplitPanel() {
    ...
    tab.payMethods.forEach(m => {
        ...
        if (inp) savedValues[m] = inp.value.replace(/[^0-9]/g, '');
    });

    let rowsHtml = '';
    tab.payMethods.forEach((m, i) => {
        const info = PAY_METHOD_INFO[m] || PAY_METHOD_INFO.cash;
        const isLast = i === tab.payMethods.length - 1;
        const val = savedValues[m] || '';
        rowsHtml += `
            <div class="split-pay-row">
                ...
                <input class="spr-input" id="split_${m}" type="text" value="${val}" placeholder="0"
                    oninput="onSplitInput('${m}')" onfocus="this.select()"
                    ${isLast ? 'readonly' : ''}>
                ${isLast ? '<span class="spr-auto-tag">Tự động</span>' : ''}
            </div>`;
    });
...
function recalcSplitRemaining() {
    ...
    let partialSum = 0;
    for (let i = 0; i < tab.payMethods.length - 1; i++) {
        const inp = document.getElementById('split_' + tab.payMethods[i]);
        partialSum += inp ? parseInt(inp.value.replace(/[^0-9]/g, '') || 0) : 0;
    }

    const lastMethod = tab.payMethods[tab.payMethods.length - 1];
    const lastInput = document.getElementById('split_' + lastMethod);
    const remaining = Math.max(0, grandTotal - partialSum);
    if (lastInput) lastInput.value = remaining > 0 ? fmtVN(remaining) : '0';

    const totalPaid = partialSum + remaining;

    const fill = document.getElementById('splitProgressFill');
    const pct = grandTotal > 0 ? Math.min((totalPaid / grandTotal) * 100, 100) : 0;
    if (fill) {
        fill.style.width = pct + '%';
        fill.className = 'split-progress-fill' + (totalPaid === grandTotal ? ' exact' : totalPaid > grandTotal ? ' over' : '');
    }

    const remEl = document.getElementById('splitRemainingVal');
    const diff = grandTotal - totalPaid;
    if (remEl) {
        remEl.textContent = diff === 0 ? '✓ Đủ tiền' : fmtVN(Math.abs(diff));
        remEl.className = 'sr-val' + (diff === 0 ? ' exact' : diff < 0 ? ' over' : '');
    }

    const payInput = document.getElementById('payAmountInput');
    if (payInput) {
        payInput.value = totalPaid;
        calculateTotals();
    }
}
...
function initBarcodeScanner() {
    let barcodeBuf = '';
    let lastKeyTime = 0;
    const SCAN_THRESHOLD = 150;  // Max ms between keypresses for scanner input
